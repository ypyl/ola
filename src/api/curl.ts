...
interface NeutralinoCurlOptions {
    debug?: boolean;
}
...
export class NeutralinoCurl {
    ...
    progress: number;
    httpHeaders: string[];
    appResourcesEXT: string;
    auth: Auth;

    constructor(opt: NeutralinoCurlOptions = {}) {
        //
        // Constructor
    ...
    async download(src: string, dst: string = "") {
        //
    ...
    async run(args: string) {
        ...
        let cmd = await os.spawnProcess(this.appResourcesEXT + `/curl/bin/curl ${args}`);

        events.on('spawnedProcess', (e: any) => {
